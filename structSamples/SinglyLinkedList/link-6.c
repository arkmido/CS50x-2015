/*
    ...
    at end, beginning, and nth position
    
    and delete nodes at nth position
    
    implement a function to reverse the list
    implement another function to reverse the list
        using recursion
*/
...
// recursive list reversing
// the head must be a global variable
void recurRev(Node *ptr)
{
    if(ptr->next == NULL)
    {
        //head = *ptr;
...
{
    // nodes at position current, previous, next
    Node *curr, *prev, *nxt;
    curr = *ptr;
    prev = NULL;

    if(list_length(*ptr) == 0)
    {
        printf("List elements must 1 or more\n");
        return;
    }
